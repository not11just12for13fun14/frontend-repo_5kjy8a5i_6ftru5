    ...
    <section id="pricing" className="relative py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="mt-12 grid lg:grid-cols-3 gap-6">
          {tiers.map((t, i) => (
            <motion.div key={t.name} initial={{ y: 16, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} viewport={{ once: true }} transition={{ delay: i * 0.05 }} className={`relative rounded-3xl p-6 border shadow-sm ${t.highlighted ? 'bg-indigo-50 border-indigo-200 dark:bg-white/10 dark:border-indigo-400/30' : 'bg-white border-slate-200 dark:bg-white/5 dark:border-white/10 dark:shadow-[0_10px_40px_rgba(2,6,23,0.4)]'}`}>
              {t.highlighted && (
              ...
              <div className="flex items-baseline justify-between">
                ...
                <div className="text-right">
                  <div className="text-3xl font-extrabold text-slate-900 dark:text-white">{t.price}</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400">{t.period}</div>
                </div>
              </div>
              ...
              <div className="mt-6">
                <button className={`w-full rounded-full px-5 py-2 text-sm font-semibold shadow ${t.highlighted ? 'text-white bg-indigo-600 hover:bg-indigo-500' : 'bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 dark:bg-white/5 dark:border-white/10 dark:text-slate-100 dark:hover:bg-white/10'}`}>Choose {t.name}</button>
              </div>
            </motion.div>
          ))}
        </div>
